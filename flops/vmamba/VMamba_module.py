...
class FrameVMamba(nn.Module):
    def __init__(self, nprompt=0, spec_h=64, spec_w=1001, patch_h=64, patch_w=4, pos_type="cut", in_chans=1,
                 ...
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0.0,
                 drop_path_rate=0.0, norm_layer=nn.LayerNorm, ssd_ngroups=1, ssd_chunk_size=256,
    ...
    def prepare_tokens(self, x, mask_index, length, mask=True):
        ...
        # Patch embedding
        mel_patches, x_embed, patch_length = self.patch_embed(x, length)  # x_embed: (B, L, D)
        H_patch = H // self.patch_embed.patch_height
        W_patch = W // self.patch_embed.patch_width
        x_embed = x_embed.view(B, H_patch, W_patch, self.embed_dim)  # reshape to 2D

        # Masking
        if mask_index is not None and mask:
            mask_index_2d = mask_index.view(B, H_patch, W_patch)
            mask_expand = mask_index_2d.unsqueeze(-1).expand(B, H_patch, W_patch, self.embed_dim).float()
            x_embed = (1 - mask_expand) * x_embed + mask_expand * self.mask_embed.expand(B, H_patch, W_patch,
                                                                                         self.embed_dim)
        # Positional embedding
        if self.pos_type == "cut":
            pos = self.pos_embed[:, 1:self.num_patches + 1, :].view(1, H_patch, W_patch, self.embed_dim)
            x_embed = x_embed + pos
        else:
            pos = self.interpolate_pos_encoding(x_embed, H, W)
            x_embed = x_embed + pos[:, 1:].view(1, H_patch, W_patch, self.embed_dim)

        x_embed = self.pos_drop(x_embed)

        return x_embed, pos, mel_patches, H_patch, W_patch, patch_length
...
def FrameVMambaModel(patch_h=64, patch_w=4, amamba2_dropout=0.1, **kwargs):
    return FrameVMamba(
        patch_h=patch_h,
        patch_w=patch_w,
        embed_dim=768,
