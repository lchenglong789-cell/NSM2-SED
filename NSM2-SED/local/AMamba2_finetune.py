...
from .data_augm import mixup_w_pretrained
...
class SEDICT(SEDTask4):
    ...
    def training_step(self, batch, batch_indx):
        ...
        amamba2_org = amamba2_feats.clone()
        ...
        if mixup_type is not None and 0.5 > random.random():
            sed_feats[weak_mask], amamba2_feats[weak_mask], labels_weak, perm_weak, c_weak = mixup_w_pretrained(
                sed_feats[weak_mask], amamba2_feats[weak_mask], labels_weak, mixup_label_type="soft"
            )
            sed_feats[strong_mask], amamba2_feats[strong_mask], labels[
                strong_mask], perm_strong, c_strong = mixup_w_pretrained(
                sed_feats[strong_mask], amamba2_feats[strong_mask], labels[strong_mask], mixup_label_type="soft"
            )
            sed_feats[unlabelled_mask], amamba2_feats[
                unlabelled_mask], _, perm_unlabelled, c_unlabelled = mixup_w_pretrained(
                sed_feats[unlabelled_mask], amamba2_feats[unlabelled_mask], labels[unlabelled_mask],
                mixup_label_type="soft"
            )
        else:
            perm_weak = None
        amamba2_feats = self.amamba2_norm(amamba2_feats)
        amamba2_org = self.amamba2_norm(amamba2_org)
        if 0.5 < random.random():
            amamba2_feats[weak_mask] = self.freq_warp(amamba2_feats[weak_mask])
            amamba2_org[weak_mask] = self.freq_warp(amamba2_org[weak_mask])
            amamba2_feats[strong_mask] = self.freq_warp(amamba2_feats[strong_mask])
            amamba2_org[strong_mask] = self.freq_warp(amamba2_org[strong_mask])
            amamba2_feats[unlabelled_mask] = self.freq_warp(amamba2_feats[unlabelled_mask])
            amamba2_org[unlabelled_mask] = self.freq_warp(amamba2_org[unlabelled_mask])
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher = self.detect(
                sed_org, amamba2_org, self.sed_teacher
            )
        ...
        if perm_weak is not None:
            ...
            loss_ict_unlabelled = self.selfsup_loss(strong_preds_student[unlabelled_mask],
                                                    strong_mix.clamp(0, 1).detach()) + \
                                  self.selfsup_loss(weak_preds_student[unlabelled_mask],
