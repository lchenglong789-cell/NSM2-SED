...
class ExponentialWarmup(BaseScheduler):
    ...
    def __init__(self, optimizer, max_lr, rampup_length, exponent=-5.0, tot_steps=None, min_lr_scaler=0.1,
                 cos_down=True):
    ...
    def _get_lr(self):
        if self.cos_down:
            if self.step_num < self.rampup_len:
                ...
                lr = [(x - x * self.min_lr_scaler) * self._get_exp_up_cos_down() + x * self.min_lr_scaler for x in
                      self.max_lr]
            return lr
        else:
            lr = [lr * self._get_scaling_factor() for lr in
                  self.max_lr]  # + [lr * self._get_exp_up_cos_down() for lr in self.max_lr[2:]]
            return lr
