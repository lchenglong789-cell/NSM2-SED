...
from .data_augm import mixup
from .scaler import TorchScaler
import numpy as np
import torchmetrics
...
class AMamba2Norm(nn.Module):
    def __init__(self):
        super(AMamba2Norm, self).__init__()
...
class SEDTask4(pl.LightningModule):
    def __init__(
        ...
        self.mel_spec = MelSpectrogram(
            sample_rate=feat_params["sample_rate"],  # 16000
            n_fft=feat_params["n_window"],  # 2048
            win_length=feat_params["n_window"],  # 2048
            hop_length=feat_params["hop_length"],  # 256
            f_min=feat_params["f_min"],  # 0
            f_max=feat_params["f_max"],  # 8000
            n_mels=feat_params["n_mels"],  # 128
            window_fn=torch.hamming_window,
            wkwargs={"periodic": False},
            power=1,
        )
        self.amamba2_norm = AMamba2Norm()
        ...
        self.get_weak_student_f1_seg_macro = torchmetrics.classification.F1Score(
            len(self.encoder.labels),
            average="macro"
        )

        self.get_weak_teacher_f1_seg_macro = torchmetrics.classification.F1Score(
            len(self.encoder.labels),
            average="macro"
        )
    ...
    def training_step(self, batch, batch_indx):
        audio, amamba2_feats, labels, padded_indxs, _ = batch
        ...
        amamba2_feats = self.amamba2_norm(amamba2_feats)
        ...
        strong_preds_student, weak_preds_student = self.detect(
            sed_feats, amamba2_feats, self.sed_student,
        )
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher = self.detect(
                sed_feats, amamba2_feats, self.sed_teacher
            )
    ...
    def validation_step(self, batch, batch_indx):
        audio, amamba2_feats, labels, padded_indxs, filenames = batch
        sed_feats = self.mel_spec(audio)
        amamba2_feats = self.amamba2_norm(amamba2_feats)
        # prediction for student
        strong_preds_student, weak_preds_student = self.detect(sed_feats, amamba2_feats, self.sed_student)
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher = self.detect(sed_feats, amamba2_feats, self.sed_teacher)
    ...
    def on_validation_epoch_end(self):
        ...
        psds1_student_sed_scores_real = self.psds1(self.val_scores_postprocessed_buffer_student_real, ground_truth,
                                                   audio_durations)
        psds2_student_sed_scores_real = self.psds2(self.val_scores_postprocessed_buffer_student_real, ground_truth,
                                                   audio_durations)
        psds1_teacher_sed_scores_real = self.psds1(self.val_scores_postprocessed_buffer_teacher_real, ground_truth,
                                                   audio_durations)
        psds2_teacher_sed_scores_real = self.psds2(self.val_scores_postprocessed_buffer_teacher_real, ground_truth,
                                                   audio_durations)
        ...
        psds1_student_sed_scores_synth = self.psds1(self.val_scores_postprocessed_buffer_student_synth, ground_truth,
                                                    audio_durations)
        psds2_student_sed_scores_synth = self.psds2(self.val_scores_postprocessed_buffer_student_synth, ground_truth,
                                                    audio_durations)
        psds1_teacher_sed_scores_synth = self.psds1(self.val_scores_postprocessed_buffer_teacher_synth, ground_truth,
                                                    audio_durations)
        psds2_teacher_sed_scores_synth = self.psds2(self.val_scores_postprocessed_buffer_teacher_synth, ground_truth,
                                                    audio_durations)
    ...
    def on_test_epoch_end(self):
        ...
        if self.evaluation:
            ...
            sed_scores_eval.io.write_sed_scores(self.test_scores_postprocessed_buffer_student,
            ...
            sed_scores_eval.io.write_sed_scores(self.test_scores_postprocessed_buffer_teacher,
