...
class CRNN(nn.Module):
    def __init__(
        ...
        self.mamba2_dropout = mamba2_dropout
        ...
        self.init_mamba2(mamba2_init)
        self.init_model(model_init, mode=mode)

        self.unfreeze_mamba2_layer = unfreeze_mamba2_layer

    def init_mamba2(self, path=None):
        if path is None:
            self.mamba2_frame = Mamba2(None, mamba2_dropout=self.mamba2_dropout)
        else:
            self.mamba2_frame = Mamba2(path, mamba2_dropout=self.mamba2_dropout)

            print("Loading mamba2 from:", path)
        self.mamba2_frame.eval()
        for param in self.mamba2_frame.parameters():
            param.detach_()
    ...
    def forward(self, x, pretrain_x, pad_mask=None, embeddings=None):
        ...
        # rnn features
        embeddings = self.mamba2_frame(pretrain_x)
    ...
    def train(self, mode=True):
        ...
        # if mode:
        #     self.mamba2_frame.eval()
        #     if self.unfreeze_mamba2_layer > 0:
        #         self.mamba2_frame.mamba2.norm_frame.train()
        #         if self.unfreeze_mamba2_layer == 14:
        #             self.mamba2_frame.train()
        #         else:
        #             unfreeze_blocks = self.unfreeze_mamba2_layer - 1
        #             for i in range(unfreeze_blocks):
        #                 block_idx = 11 - i
        #                 self.mamba2_frame.mamba2.blocks[block_idx].train()
