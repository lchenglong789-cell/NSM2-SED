...
class CRNN(nn.Module):
    def __init__(
        ...
        self.init_vim(vim_init)
        self.init_model(model_init, mode=mode)

        self.unfreeze_vim_layer = unfreeze_vim_layer

    def init_vim(self, path=None):
        if path is None:
            self.vim_frame = VimMamba2(None, vim_dropout=self.vim_dropout)
        else:
            self.vim_frame = VimMamba2(path, vim_dropout=self.vim_dropout)

            print("Loading vim from:", path)
        self.vim_frame.eval()
        for param in self.vim_frame.parameters():
            param.detach_()
    ...
    def forward(self, x, pretrain_x, pad_mask=None, embeddings=None):
        ...
        # rnn features
        embeddings = self.vim_frame(pretrain_x)
    ...
    def train(self, mode=True):
        ...
        # if mode:
        #     self.vim_frame.eval()
        #     if self.unfreeze_vim_layer > 0:
        #         self.vim_frame.vim.norm_frame.train()
        #         if self.unfreeze_vim_layer == 14:
        #             self.vim_frame.train()
        #         else:
        #             unfreeze_blocks = self.unfreeze_vim_layer - 1
        #             for i in range(unfreeze_blocks):
        #                 block_idx = 11 - i
        #                 self.vim_frame.vim.blocks[block_idx].train()
