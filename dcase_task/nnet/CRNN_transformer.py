...
import torch
from .transformer.transformer_model import trans
...
class CRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.init_trans(trans_init)
        self.init_model(model_init, mode=mode)
        
        self.unfreeze_trans_layer = unfreeze_trans_layer

    def init_trans(self, path=None):
        if path is None:
            self.trans_frame = trans(None, trans_dropout=self.trans_dropout)
        else:
            self.trans_frame = trans(path, trans_dropout=self.trans_dropout)
            
            print("Loading trans from:", path)
        self.trans_frame.eval()
        for param in self.trans_frame.parameters():
            param.detach_()
    ...
    def forward(self, x, pretrain_x, pad_mask=None, embeddings=None):
        ...
        # rnn features
        embeddings = self.trans_frame(pretrain_x)
    ...
    def train(self, mode=True):
        ...
        # either eval the trans-Frame or train the whole model
        # if mode:
        #     self.trans_frame.eval()
        #     if self.unfreeze_trans_layer > 0:
        #         self.trans_frame.trans.norm_frame.train()
        #         if self.unfreeze_trans_layer == 14:
        #             self.trans_frame.train()
        #         else:
        #             unfreeze_blocks = self.unfreeze_trans_layer - 1
        #             for i in range(unfreeze_blocks):
        #                 block_idx = 11 - i
        #                 self.trans_frame.trans.blocks[block_idx].train()
