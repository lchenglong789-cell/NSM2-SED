...
class AMamba2Transform:
    def __init__(self):
        self.transform = MelSpectrogram(16000, f_min=60, f_max=7800, hop_length=160, win_length=1024, n_fft=1024,
                                        n_mels=64)
        self.to_db = AMamba2Norm()
...
class StronglyAnnotatedSet(Dataset):
    def __init__(
        ...
        self.amamba2_transform = AMamba2Transform()
    ...
    def __getitem__(self, item):
        ...
        amamba2_feat = self.amamba2_transform(mixture)
        out_args = [mixture, amamba2_feat, strong.transpose(0, 1), padded_indx]
...
class WeakSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # sed_feat = self.sed_transform(mixture)
        amamba2_feat = self.amamba2_transform(mixture)
        out_args = [mixture, amamba2_feat, weak.transpose(0, 1), padded_indx]
