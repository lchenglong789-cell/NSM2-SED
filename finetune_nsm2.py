...
from dcase_task4.dataio.sampler import ConcatDatasetBatchSampler
from dcase_task4.dataio.datasets_amamba2_sed import StronglyAnnotatedSet, UnlabeledSet, WeakSet
from dcase_task4.utils.encoder import ManyHotEncoder
from dcase_task4.nnet.CRNN_amamba2 import CRNN
from local.scheduler import ExponentialWarmup
from local.classes_dict import classes_labels
from local.nsm2_finetune import SEDICT
...
def separate_opt_params(model):
    ...
    for k, p in model.named_parameters():
        if "amamba2_frame" not in k:
            ...
            if "blocks.0." in k:
                mamba_params[1].append(p)
            elif "blocks.1." in k:
                mamba_params[2].append(p)
            elif "blocks.2." in k:
                mamba_params[3].append(p)
            elif "blocks.3." in k:
                mamba_params[4].append(p)
            elif "blocks.4." in k:
                mamba_params[5].append(p)
            elif "blocks.5." in k:
                mamba_params[6].append(p)
            elif "blocks.6." in k:
                mamba_params[7].append(p)
            elif "blocks.7." in k:
                mamba_params[8].append(p)
            elif "blocks.8" in k:
                mamba_params[9].append(p)
            elif "blocks.9." in k:
                mamba_params[10].append(p)
            elif "blocks.10." in k:
                mamba_params[11].append(p)
            elif "blocks.11." in k:
                mamba_params[12].append(p)
            elif ".norm_frame." in k:
                mamba_params[13].append(p)
            else:
                mamba_params[0].append(p)
    return cnn_params, rnn_params, list(reversed(mamba_params))
...
def single_run(
    ...
    ##### model definition  ############
    sed_student = CRNN(
        unfreeze_amamba2_layer=config["opt"]["mamba_trainable_layers"],
        **config["net"],
        model_init=config["ultra"]["model_init"],
        amamba2_init=config["ultra"]["amamba2_init"],
        amamba2_dropout=config["ultra"]["amamba2_dropout"],
        mode="student")
    sed_teacher = CRNN(
        unfreeze_amamba2_layer=config["opt"]["mamba_trainable_layers"],
        **config["net"],
    ...
    if test_state_dict is None:
        ...
        cnn_params, rnn_params, mamba_params = separate_opt_params(sed_student)
        ...
        mamba_trainable_params = []
        trainable_layers = config["opt"]["mamba_trainable_layers"]
        for i in range(trainable_layers):
            mamba_trainable_params.append(mamba_params[i])
            for p in mamba_params[i]:
                p.requires_grad = True

        init_lr = config["opt"]["mamba_lr"]
        lr_scale = config["opt"]["mamba_lr_scale"]
        scale_lrs = [init_lr * (lr_scale ** i) for i in range(trainable_layers)]
        mamba_param_groups = [{"params": mamba_trainable_params[i], "lr": scale_lrs[i]} for i in
                            range(len(mamba_trainable_params))]
        max_lrs = [cnn_param_groups[0]["lr"]] + [rnn_param_groups[0]["lr"]] + [x["lr"] for x in mamba_param_groups]

        param_groups = cnn_param_groups + rnn_param_groups + mamba_param_groups
    ...
    trainer = pl.Trainer(
        ...
        devices=devices,
        strategy=DDPStrategy(find_unused_parameters=True),  # config["training"].get("backend"),
...
def prepare_run(argv=None):
    ...
    parser.add_argument(
        "--conf_file",
        default="/home/02363-2/SED/again/TLstudy/confs/AMamba2_finetune_14.yaml",
        help="The configuration file with all the experiment parameters.",
    )
    parser.add_argument(
        "--log_dir",
        default="/data/LCL/workplace/new90/mamba_sed/pre_stage2_14/",
    ...
    if args.mamba_lr_scale is not None:
        configs["opt"]["mamba_lr_scale"] = args.mamba_lr_scale
    if args.loss_weight is not None:
        configs["training"]["const_max"] = args.loss_weight
    if args.lr is not None:
        configs["opt"]["mamba_lr"] = args.lr
