...
class CheckpointEveryNSteps(pl.Callback):
    # Allows model checkpoints to be saved at fixed training steps instead of the default based on validation loss
    def __init__(
            self,
            save_step_frequency,
...
class CheckpointEveryNEpochs(pl.Callback):
    # Model checkpoints are saved at regular training intervals (epochs).
    def __init__(
            self,
...
class WeightDecayCosineScheduler(pl.Callback):
    # Dynamically adjusts the weight attenuation value based on the cosine curve,
    # smoothly transitioning from the initial value wd_init to the final value wd_final
    def __init__(
            self,
            wd_init,
            wd_final
