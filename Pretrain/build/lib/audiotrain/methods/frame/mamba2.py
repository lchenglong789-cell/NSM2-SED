...
from audiotrain.models.amamba2.audio_mamba2 import get_num_patches

random.seed(1234)


class FrameAMamba2TrainTransform:
    def __init__(self, sr=16000, win_length=1024, aug_tea=True, aug_stu=True, freq_wrap=True, mask_ratio=0.75,
                 mask_nooverlap=False, min_mask_len=2, mask_len=5, mask_type="random", anchor_len=6., patch_h=64,
                 patch_w=4, n_mels=64, **kwargs):
    ...
    def __call__(self, input):
        ...
        if self.mask_type == "random":
            ...
            mask = random_mask.get_mask(1, num_patches, self.mask_ratio, no_overlap=self.mask_nooverlap,
                                        min_length=self.mask_len, type="static", other=self.min_mask_len).squeeze(0)
        else:
            mask = random_mask.get_mask(1, num_patches, self.mask_ratio, no_overlap=self.mask_nooverlap,
                                        min_length=self.mask_len, type="uniform", other=self.min_mask_len).squeeze(0)
