...
class DistributedSamplerWrapper(DistributedSampler):
    ...
    def __iter__(self):
        ...
        original_generator = self.sampler.generator
        ...
        self.sampler.generator = original_generator
        indices = indices[self.rank:self.total_size:self.num_replicas]
        return iter(indices)
    ...
    #     self.sampler.generator.manual_seed(self.seed + self.epoch)
    #     indices = list(self.sampler)
    #     valid_indices = [idx for idx in indices if idx < len(self.dataset)]
    #     if len(valid_indices) < len(indices):
    #         warnings.warn(f"{len(indices) - len(valid_indices)} is not using index")
    #     if self.epoch == 0:
    #         print(f"\n DistributedSamplerWrapper :  {valid_indices[:10]} \n\n")
    #     valid_indices = valid_indices[self.rank:self.total_size:self.num_replicas]
...
class DownstreamDataModule(LightningDataModule):
    def __init__(self,
        ...
        dataset_info_b = datasets.get_dataset("audioset_b")
        ...
        if num_folds > 1:
            ...
            if self.dataset_name == "audioset":
                dataset_ub = dataset_info.creator(data_path,
                                                  ...
                                                  transforms[0],
                                                  target_transform=target_transforms[0])
                dataset_b = dataset_info_b.creator("/data/AudioSet/audioset/manifest_b/",
                                                   "train",
                                                   transforms[0],
                                                   target_transform=target_transforms[0])
    ...
    def train_dataloader(self):
        ...
        if self.dataset_name == "dcase":
            ...
            return data.DataLoader(
                ...
                num_workers=self.num_workers,
                shuffle=False,
                sampler=DistributedSamplerWrapper(self.dataset_train,
                                                  self.sampler, ),
                drop_last=True,
                collate_fn=self.collate_fn,
