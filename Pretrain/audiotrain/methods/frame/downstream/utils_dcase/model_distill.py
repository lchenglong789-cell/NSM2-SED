...
from audiotrain.datasets.dcase_utils import ManyHotEncoder
from audiotrain.utils.common import cosine_scheduler_epoch
from audiotrain.methods.frame.downstream.utils_dcase.class_dict import classes_labels
from audiotrain.methods.frame.downstream.utils_psds_eval.gpu_decode import (
...
from audiotrain.methods.frame.downstream.utils_psds_eval.evaluation import (
    compute_per_intersection_macro_f1,
    compute_psds_from_operating_points
)

'''
This file is modified from models.py
'''
...
class DistillPLModule(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if torch.any(mask_synth):
            ...
            decoded_student_strong = decode_preds(strong_pred[mask_synth], self.config["training"]["val_thresholds"],
                                                  self.median_filter)
            decoded_teacher_strong = decode_preds(teacher_strong[mask_synth], self.config["training"]["val_thresholds"],
                                                  self.median_filter)
