...
from audiotrain.methods.frame.audio_mamba2 import FrameASM_small, FrameASM_base
from audiotrain.methods.frame.byol import MultiCropWrapper, ByolLoss
...
class FrameAMamba2(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        if arch == "small":
            encoder_fn = FrameASM_small
            embed_dim = 384
        elif arch == "base":
            encoder_fn = FrameASM_base
            embed_dim = 768
        else:
            raise RuntimeError("arch {} is not implemented".format(arch))
        self.symmetric = symmetric
        if avg_blocks == 0:  # frame
...
class FrameAMamba2LightningModule(LightningModule):
    def __init__(self,
                 arch="small",
        ...
        super().__init__()
        self.model = FrameAMamba2(arch=arch,
                                  symmetric=symmetric,
                                  pos_type=pos_type,
        ...
        # self.ema_scheduler = cosine_scheduler_step(
        #     base_value=ema,
        #     final_value=1.0,
        #     max_steps=max_steps,
        #     warmup_steps=0
        ...
        # self.wd_scheduler = cosine_scheduler_step(
        #     base_value=0.4,
        #     final_value=0.04,
        #     max_steps=max_steps,
        #     warmup_steps=0
    ...
    def add_model_specific_args(parent_parser):
        parser = parent_parser.add_argument_group("FrameAMamba2Model")
        ...
        parser.add_argument('--avg_blocks', default=0, type=int,
                            help="0 means frame, a positive int value means data2vec style loss")
