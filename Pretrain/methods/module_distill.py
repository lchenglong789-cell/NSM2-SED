...
from audiotrain.methods.frame.audio_mamba2 import FrameASM_base
from audiotrain.utils.common import cosine_scheduler_step, get_params_groups
from audiotrain.modules.head import LinearHead
from audiotrain.mambas.common import Normalize, MinMax, RandomCrop, Identity
from audiotrain.mambas.byol_a import Mixup, RandomResizeCrop
from audiotrain.methods.frame.byol import build_mlp
from audiotrain.methods.clip.downstream.utils import Metric
from audiotrain.lightning.datamodules import DistributedSamplerWrapper
from audiotrain.methods.clip.downstream.model import PretrainedEncoderPLModule as ClipEncoder
...
class DistillAMamba2TrainTransform:
...
class DistllAMamba2TargetTransform:
...
class Distill(nn.Module):
    def __init__(self, ncls=0, project=False):
        ...
        self.student = FrameASM_base(nprompt=ncls)
...
def layer_wise_lr_groups(model):
    ...
    for name, param in model.named_parameters():
        ...
        # encoder.norm_frame
        # head
        name = name.replace("models.student", "encoder")
        if not param.requires_grad:
            continue

        if name.startswith("encoder.mask_embed") or name.startswith("encoder.pos_embed") or name.startswith(
                "encoder.patch_embed"):
            ...
            # 'name':name})
        elif name.startswith("models.linear") or name.startswith("models.project"):
            groups.append({'params': param,
...
class DistillLightningModule(LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        parser = parent_parser.add_argument_group("DistillAMamba2Model")
...
from audiotrain.datasets import LMDBDataset
from torch.utils import data


class DistillAMamba2DataModule(LightningDataModule):
    def __init__(self,
                 data_path=None,
        ...
        _dataset_ub = LMDBDataset(data_path,
                                  split="train",
                                  subset=subset,
                                  transform=DistillAMamba2TrainTransform())
        ...
        dataset_ub = LMDBDataset(data_path,
                                 ...
                                 subset=subset,
                                 transform=DistillAMamba2TrainTransform(),
                                 target_transform=DistllAMamba2TargetTransform(_dataset))
        ...
        s = torch.load(os.path.join("/home/02363-2/SED/audiotrain/sampler/weights_labels.pt"), map_location="cpu")
    ...
    def add_data_specific_args(parent_parser):
        parser = parent_parser.add_argument_group("DistillAMamba2Data")
